...
public class EnumSelectorBuilder<T extends Enum<?>> extends FieldBuilder<T, EnumListEntry<T>> {
   ...
   private Function<Enum<?>, ITextComponent> enumNameProvider;
   ...
   public EnumListEntry<T> build() {
      EnumListEntry<T> entry = new EnumListEntry<T>(
        this.getFieldNameKey(), this.clazz, this.value, this.getResetButtonKey(), this.defaultValue,
        this.saveConsumer, this.enumNameProvider, null, this.isRequireRestart());
