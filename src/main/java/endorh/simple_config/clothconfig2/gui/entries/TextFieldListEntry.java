...
public abstract class TextFieldListEntry<T>
  extends TooltipListEntry<T> {
	...
	protected TextFieldListEntry(
	  ITextComponent fieldName, T original, ITextComponent resetButtonKey, Supplier<T> defaultValue
	) {
	  ...
	  Supplier<Optional<ITextComponent[]>> tooltipSupplier, boolean requiresRestart
		    ...
		    Minecraft.getInstance().fontRenderer, 0, 0, 148, 18, NarratorChatListener.EMPTY) {
			  ...
			  public void render(@NotNull MatrixStack matrices, int int_1, int int_2, float float_1) {
				  this.setFocused(TextFieldListEntry.this.isSelected &&
				                  TextFieldListEntry.this.getListener() == this);
				  TextFieldListEntry.this.textFieldPreRender(this);
				  super.render(matrices, int_1, int_2, float_1);
			  }
			  
			  public void writeText(@NotNull String string_1) {
		...
		this.resetButton =
		  new Button(0, 0, Minecraft.getInstance().fontRenderer.getStringPropertyWidth(
			 resetButtonKey) + 6, 20, resetButtonKey, this::onReset);
		this.widgets = Lists.newArrayList(this.textFieldWidget, this.resetButton);
	}
	
	protected void onReset(Button resetButton) {
	...
	public void render(
	  ...
	  int mouseY, boolean isHovered, float delta
		...
		this.resetButton.active = this.isEditable() && this.getDefaultValue().isPresent() &&
		                          !this.isMatchDefault(this.textFieldWidget.getText());
		...
		TextFieldListEntry.setTextFieldWidth(
		  this.textFieldWidget, 148 - this.resetButton.getWidth() - 4);
