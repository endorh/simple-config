...
public class KeyCodeEntry extends TooltipListEntry<ModifierKeyCode> {
   ...
   public KeyCodeEntry(ITextComponent fieldName, ModifierKeyCode value, ITextComponent resetButtonKey, Supplier<ModifierKeyCode> defaultValue, Consumer<ModifierKeyCode> saveConsumer, Supplier<Optional<ITextComponent[]>> tooltipSupplier, boolean requiresRestart) {
      ...
      this.buttonWidget = new Button(0, 0, 150, 20, NarratorChatListener.EMPTY, (widget) -> {
         this.getConfigScreen().setFocusedBinding(this);
      });
   ...
   public void render(MatrixStack matrices, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isHovered, float delta) {
      ...
      this.resetButton.active = this.isEditable() && this.getDefaultValue().isPresent() && !this.getDefaultValue().get()
      ...
      if (this.getConfigScreen().getFocusedBinding() == this) {
         this.buttonWidget.setMessage((new StringTextComponent("> ")).mergeStyle(TextFormatting.WHITE).append(this.buttonWidget.getMessage().copyRaw().mergeStyle(TextFormatting.YELLOW)).append((new StringTextComponent(" <")).mergeStyle(TextFormatting.WHITE)));
      }
