...
import endorh.simple_config.clothconfig2.gui.*;
...
import endorh.simple_config.clothconfig2.gui.widget.PresetPickerWidget.Preset.TypeWrapper;
...
import static endorh.simple_config.clothconfig2.gui.WidgetUtils.forceUnFocus;
...
public class PresetPickerWidget extends FocusableGui {
	...
	public PresetPickerWidget(
	  AbstractConfigScreen screen, int x, int y, int w
		  ...
		  .tooltip(() -> {
			  ...
			  return Lists.newArrayList(
			    new TranslationTextComponent(String.format(
					"simple-config.preset.load.%s.%s",
					value.server ? "server" : "client",
					value.remote ? "remote" : "local")));
		  }), new TranslationTextComponent("simple-config.preset.load.label"));
		...
		provider.setPlaceholder(new TranslationTextComponent("simple-config.ui.no_presets_found"));
	...
	protected List<ITextComponent> getSaveTooltip(
	  boolean delete, boolean remote
		...
		final SnapshotHandler handler = getHandler();
		...
		final ArrayList<ITextComponent> tt = Lists.newArrayList(
		  new TranslationTextComponent(String.format(
		    "simple-config.preset.%s.%s.%s",
		    delete? "delete" : "save", server ? "server" : "client",
			 remote? "remote" : "local")));
		if (!delete && !remote && handler.canSaveRemote())
			tt.add(new TranslationTextComponent("simple-config.preset.save.remote.shift"));
		if (!delete && selector.getValue() != null)
			tt.add(new TranslationTextComponent("simple-config.preset.delete.alt"));
		return tt;
	}
	...
	protected SnapshotHandler getHandler() {
	...
	public void load(Preset preset) {
		...
		final SnapshotHandler handler = getHandler();
		screen.getHistory().startBatch(screen, null); {
			if (preset.remote) {
				...
				screen.addDialog(
				  new ProgressDialog(
					 screen, new TranslationTextComponent(
						"simple-config.preset.dialog.loading.title"),
					 splitTtc("simple-config.preset.dialog.loading.remote.body", preset.name),
					 future));
			} else handler.restore(handler.getLocal(preset.name, type), type);
		} screen.getHistory().saveBatch(screen);
	}
	...
	public void save(String name, boolean remote, boolean overwrite) {
		...
		final SnapshotHandler handler = getHandler();
		final CommentedConfig preserved = handler.preserve(type);
		...
		if (!overwrite && !userSkipOverwriteDialog && presetMap.containsKey(name)
			...
			screen.addDialog(Util.make(new ConfirmDialog(
			  screen, new TranslationTextComponent("simple-config.preset.dialog.overwrite.title"),
			  (b, cs) -> {
				  if (b) {
					  if (cs[0]) userSkipOverwriteDialog = true;
					  save(name, remote, true);
				  }
			  }, splitTtc("simple-config.preset.dialog.overwrite."
			              + (remote ? "remote." : "local.")
			              + (server ? "server" : "client"), name),
			  DialogTexts.GUI_CANCEL, new TranslationTextComponent(
			  "simple-config.preset.dialog.overwrite.confirm"),
			  new CheckboxButton(false, 0, 0, 0, new TranslationTextComponent(
				 "simple-config.ui.do_not_ask_again_this_session"), null)
			), d -> d.setConfirmButtonTint(0xAAAA00AA)));
		} else if (!overwrite && otherMap.containsKey(name) && !presetMap.containsKey(name)) {
			final String flavor = remote ? "remote" : "local";
			screen.addDialog(Util.make(new ConfirmDialog(
			  screen, new TranslationTextComponent("simple-config.preset.dialog.mistaken.title"),
			  b -> {
				 if (b) save(name, remote, true);
			  }, splitTtc("simple-config.preset.dialog.mistaken." + flavor + ".body", name)), d -> {
				d.setConfirmText(new TranslationTextComponent(
				  "simple-config.preset.dialog.mistaken.option." + flavor + ".create"));
				d.setConfirmButtonTint(remote? 0xAA429090 : 0xAA429042);
				d.addButton(1, Util.make(new TintedButton(0, 0, 0, 20, new TranslationTextComponent(
				  "simple-config.preset.dialog.mistaken.option." + flavor + ".overwrite"),  p -> {
					save(name, !remote, true);
					d.cancel(false);
				}), b -> b.setTintColor(remote ? 0xAA429042 : 0xAA429090)));
			}));
		} else if (remote) {
			final CompletableFuture<Void> future =
			  handler.saveRemote(name, type, preserved);
			screen.addDialog(Util.make(new ProgressDialog(
			  screen, new TranslationTextComponent(
				 "simple-config.preset.dialog.saving.title"),
			  splitTtc("simple-config.preset.dialog.saving.remote.body", name),
			  future
			), d -> d.setCancellableByUser(false)));
			// Once the packet has been sent, cancellation is not possible
		} else {
			handler.saveLocal(name, type, preserved).ifPresent(e ->
			  screen.addDialog(new ErrorDialog(
				 screen, new TranslationTextComponent(
					"simple-config.preset.dialog.saving.error.title"),
				 e, splitTtc("simple-config.preset.dialog.saving.error.local.body", name)))
			);
		}
	}
	...
	public void delete(Preset preset, boolean skipConfirm) {
		...
		if (!userSkipConfirmDeleteDialog && !skipConfirm) {
			screen.addDialog(Util.make(new ConfirmDialog(
			  screen, new TranslationTextComponent(
				 "simple-config.preset.dialog.delete.confirm.title"),
			  (b, cs) -> {
				  if (b) {
					  if (cs[0]) userSkipConfirmDeleteDialog = true;
					  delete(preset, true);
				  }
			  }, splitTtc("simple-config.preset.dialog.delete.confirm."
			              + (preset.remote? "remote." : "local.")
			              + (preset.server? "server" : "client"), preset.name),
			  new CheckboxButton(false, 0, 0, 0, new TranslationTextComponent(
				 "simple-config.ui.do_not_ask_again_this_session"), null)), d -> {
				d.setConfirmText(new TranslationTextComponent(
				  "simple-config.preset.dialog.delete.confirm.delete", preset.name));
			...
			screen.addDialog(Util.make(new ProgressDialog(
			  screen, new TranslationTextComponent(
			  "simple-config.preset.dialog.deleting.title"),
			  splitTtc("simple-config.preset.dialog.deleting.remote.body", preset.name),
			  future), d -> d.setCancellableByUser(false)));
			  ...
			  @NotNull String text
				...
				if (localPresets.containsKey(text)) {
					...
					if (remotePresets.containsKey(text)) {
					  ...
					  "simple-config.config.error.unknown_preset")));
				}
			}
