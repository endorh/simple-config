  ...
  extends AbstractConfigEntry<T> {
	...
	public Rectangle getEntryArea(int x, int y, int entryWidth, int entryHeight) {
		return new Rectangle(
		  this.getParent().left, y, this.getParent().right - this.getParent().left,
		  this.getItemHeight() - 4);
	}
	
	public boolean isMouseInside(
	  int mouseX, int mouseY, int x, int y, int entryWidth, int entryHeight
	) {
		return this.getParent().isMouseOver(mouseX, mouseY) &&
		       this.getEntryArea(x, y, entryWidth, entryHeight).contains(mouseX, mouseY);
	}
