...
public class DemoDeclarativeConfigCategory {
	...
	@Text private static final Component _greeting = ttc(
	  prefix("text.greeting_declarative"),
	  ttc(prefix("text.github")).withStyle(s -> s
		 .withColor(ChatFormatting.AQUA)
		 .withClickEvent(new ClickEvent(
			ClickEvent.Action.OPEN_URL,
			"https://github.com/endorh/simple-config/blob/1.18/src/main/java/endorh/simpleconfig/demo/DemoDeclarativeConfigCategory.java"))));
	...
	public abstract static class demo_group {
		...
		@Bind private static List<Component> even_score$tooltip() {
			return asList(
		...
		@Text private static final Supplier<Component> _2 = () ->
		  ttc(prefix("text.some_complex_text"),
		      stc(summon_command).withStyle(s -> s
		        // If you're planning to have many clickable links,
		        //   you may want to create a wrapper method for this
		        .withColor(ChatFormatting.LIGHT_PURPLE)
			     .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, ttc("chat.copy.click")))
			     .withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, summon_command))));
		...
		@Group public abstract static class nested_group {
			...
			@Entry public static Pair<
			  @Default("overwritten") @Length(max=10) String,
