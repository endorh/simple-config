		  ...
		  .text(ttc(
			 ...
			 ttc(prefix("text.github")).withStyle(s -> s
			   ...
			   .withClickEvent(new ClickEvent(
				  ...
				  "https://github.com/endorh/simple-config/blob/1.17/src/main/java/endorh/simpleconfig/demo/DemoConfigCategory.java")))))
	...
	public static class StringIntPairSerializer implements ConfigEntrySerializer<Pair<String, Integer>> {
		...
		@Override public @NotNull TextFormatter getConfigTextFormatter() {
			return text -> {
				...
				MutableComponent c = TextComponent.EMPTY.copy();
				if (!split.isEmpty()) c = c
				  .append(new TextComponent(split.get(0)).withStyle(ChatFormatting.DARK_GREEN));
				if (split.size() > 1) {
					...
					c = c.append(new TextComponent(",").withStyle(ChatFormatting.GOLD))
					  .append(new TextComponent(split.get(1)).withStyle(numberStyle));
				}
				for (int i = 2; i < split.size(); i++) c = c
				  .append(new TextComponent("," + split.get(i))
