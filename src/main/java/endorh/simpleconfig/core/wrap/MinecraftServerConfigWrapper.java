...
public class MinecraftServerConfigWrapper {
	...
	public static void onLoadComplete(FMLLoadCompleteEvent event) {
		DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> MinecraftServerConfigWrapper::wrapMinecraftServerConfig);
	}
	...
	public static class MinecraftGameRulesWrapperBuilder {
		...
		private void addServerPropertiesEntries() {
			...
			with(group("world-gen"), () -> {
				...
				add("level-type", string(WorldPresets.NORMAL.location().toString()).suggest(Stream.of(
				  WorldPresets.NORMAL, WorldPresets.FLAT,
		...
		private <T> AbstractConfigEntryBuilder<T, ?, ?, ?, ?, ?> patch(
		  Key<?> key, ConfigEntryBuilder<T, ?, ?, ?> entryBuilder
			...
			entryBuilder = entryBuilder.tooltip(() -> Stream.concat(
			  ...
			  Stream.of(Component.literal("/gamerule " + key.getId()).withStyle(ChatFormatting.GRAY))
