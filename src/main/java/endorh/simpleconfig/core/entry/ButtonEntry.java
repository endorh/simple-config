...
public class ButtonEntry extends GUIOnlyEntry<
  Consumer<ConfigEntryHolder>, Runnable, ButtonEntry> {
	...
	> implements ButtonEntryBuilder {
		...
		@Override @Contract(pure=true) public @NotNull Builder label(ITextComponent label) {
			Builder copy = copy();
			copy.buttonLabelSupplier = () -> label;
			return copy;
		}
		
		@Override @Contract(pure=true) public @NotNull Builder label(Supplier<ITextComponent> label) {
			Builder copy = copy();
			copy.buttonLabelSupplier = label;
			return copy;
		}
