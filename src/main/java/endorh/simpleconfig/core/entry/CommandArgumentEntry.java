...
> extends AbstractConfigEntry<ParsedArgument<A>, String, ParsedArgument<A>> {
   ...
   > extends AbstractConfigEntryBuilder<
      ParsedArgument<A>, String, ParsedArgument<A>, CommandArgumentEntry<A, T>,
      CommandArgumentEntryBuilder<A, T>, Builder<A, T>
   > implements CommandArgumentEntryBuilder<A, T> {
      ...
      @Override protected CommandArgumentEntry<A, T> buildEntry(
         ConfigEntryHolder parent, String name
      ) {
         return new CommandArgumentEntry<>(parent, name, value, aType);
      }

      @Override protected Builder<A, T> createCopy(ParsedArgument<A> value) {
   ...
   public static class ParsedArgumentTypeWrapper<A> implements TypeWrapper<ParsedArgument<A>> {
      ...
      @Override public Pair<Optional<ParsedArgument<A>>, Optional<ITextComponent>> parseElement(@NotNull String text) {
         try {
            return Pair.of(Optional.of(ParsedArgument.parse(aType, text)), Optional.empty());
         } catch (CommandSyntaxException e) {
            return Pair.of(Optional.empty(), Optional.of(new StringTextComponent(
               e.getMessage()).modifyStyle(s -> s.applyFormatting(TextFormatting.RED))));
         }
      }

      @Override public ITextComponent getDisplayName(@NotNull ParsedArgument<A> element) {
         return new StringTextComponent(element.text());
      }
