...
public class OptionEntry<T> extends AbstractConfigEntry<T, String, T> {
	...
	public Function<T, ITextComponent> displayer;
	...
	> implements OptionEntryBuilder<T> {
		...
		Function<T, ITextComponent> displayer = t -> new StringTextComponent(t.toString());
		...
		@Override public @NotNull OptionEntryBuilder<T> withDisplay(Function<T, ITextComponent> display) {
			Builder<T> copy = copy();
		...
		@Override protected OptionEntry<T> buildEntry(ConfigEntryHolder parent, String name) {
			...
			entry.deserializer = deserializer != null? deserializer : s ->
			  entry.valueSupplier.get().stream()
