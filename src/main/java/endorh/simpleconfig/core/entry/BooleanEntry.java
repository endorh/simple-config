...
public class BooleanEntry
  extends AbstractConfigEntry<Boolean, Boolean, Boolean> implements AtomicEntry<Boolean> {
	...
	@Override public boolean addCommandSuggestions(SuggestionsBuilder builder) {
		...
		Message msg = null;
		if (defValue) {
			msg = new TranslationTextComponent("simpleconfig.command.suggest.default");
		} else if (current) msg = new TranslationTextComponent("simpleconfig.command.suggest.current");
		if (msg != null) {
