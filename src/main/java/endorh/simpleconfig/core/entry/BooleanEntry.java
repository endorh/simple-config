  ...
  extends AbstractConfigEntry<Boolean, Boolean, Boolean> implements IKeyEntry<Boolean> {
	...
	> implements BooleanEntryBuilder {
		...
		@Override @Contract(pure=true) public @NotNull Builder text(String translation) {
			Builder copy = copy();
			final MutableComponent yes = Component.translatable(translation + ".true");
			final MutableComponent no = Component.translatable(translation + ".false");
	...
	@Override public boolean addCommandSuggestions(SuggestionsBuilder builder) {
		...
		Message msg = null;
		if (defValue) {
			msg = Component.translatable("simpleconfig.command.suggest.default");
		} else if (current) msg = Component.translatable("simpleconfig.command.suggest.current");
		if (msg != null) {
