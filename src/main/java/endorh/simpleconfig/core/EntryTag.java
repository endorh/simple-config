...
import static endorh.simpleconfig.core.SimpleConfigTextUtil.toTitleCase;
...
public class EntryTag implements Comparable<EntryTag> {
	...
	public static EntryTag coloredTag(TextFormatting color) {
		...
		return TAGS.computeIfAbsent(color, c -> of(
		  500 + color.getColorIndex(), name + " tag", Entries.TAG.withTint(color), () -> splitTtc(
	...
	public static EntryTag coloredBookmark(TextFormatting color) {
		...
		return BOOKMARKS.computeIfAbsent(color, c -> of(
		  600 + color.getColorIndex(), name + " bookmark", Entries.BOOKMARK.withTint(color), () -> splitTtc(
	...
	public static EntryTag copyTag(int order, String text, String tooltipTranslationKey) {
		return copyTag(order, text, () -> splitTtc(tooltipTranslationKey));
	}
	...
	public static EntryTag translated(
	  ...
	  @Nullable Consumer<Integer> clickAction
		...
		return of(order, comment, icon, () -> splitTtc(tooltipTranslationKey), clickAction);
	}
