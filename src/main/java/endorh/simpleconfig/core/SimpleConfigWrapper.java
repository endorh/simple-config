...
public class SimpleConfigWrapper {
		...
		for (ModConfig extra: extraFiles) {
			...
			ConfigCategoryBuilder categoryBuilder = category(key)
			  ...
			  .withDescription(() -> splitTtc(
				 ...
				 Component.literal(extra.getFileName()).withStyle(ChatFormatting.DARK_AQUA)));
	  ...
	  ConfigEntryBuilder<?, ?, ?, ?> builder, @Nullable ValueSpec spec, @Nullable Predicate<Object> validator
		...
		if (spec != null) {
			...
			builder = builder.configError(t -> !spec.test(t)? Optional.of(
				Component.translatable("simpleconfig.config.error.invalid_value_generic", t)) : Optional.empty());
	...
	static {
		...
		reg(CommentedConfig.class, Function.identity(),
			(s, v, t) -> guessMapType(List.of(v), t));
	}
	  ...
	  List<?> defValue, Predicate<Object> validator
		...
		if (defValue.isEmpty()) {
			...
			if (allInstance(defValue, Boolean.class)) {
				...
				//noinspection unchecked
				b = list(guessMapType((List<CommentedConfig>) defValue, o -> validator.test(List.of(o))));
