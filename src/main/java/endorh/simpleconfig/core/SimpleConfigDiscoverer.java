	...
	private static @Nullable IEventBus tryGetEventBus(@Nullable ModContainer container) {
		...
		try {
			...
			if (IEventBus.class.isAssignableFrom(method.getReturnType())
				&& method.invoke(container) instanceof IEventBus modEventBus)
				return modEventBus;
		...
		try {
			...
			if (IEventBus.class.isAssignableFrom(field.getType())
				&& field.get(container) instanceof IEventBus modEventBus)
				return modEventBus;
		...
		try {
			...
			for (ModFileScanData scanData: modList.getAllScanData()) {
				...
				List<AnnotationData> ccTargets = scanData.getAnnotations().stream()
				  .filter(d -> CONFIG_CLASS_TYPE.equals(d.annotationType()))
				  .toList();
				for (AnnotationData d: ccTargets) {
					Map<String, Object> args = d.annotationData();
					...
					String className = d.clazz().getClassName();
			...
			throw new ReportedException(CrashReport.forThrowable(e, "Error discovering config classes"));
		}
	}
