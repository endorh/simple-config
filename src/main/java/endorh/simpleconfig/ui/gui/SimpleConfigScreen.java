...
@OnlyIn(Dist.CLIENT) public class SimpleConfigScreen
  extends AbstractConfigScreen implements ISearchHandler {
	  ...
	  ResourceLocation backgroundLocation
		...
		navigateUpButton = MultiFunctionImageButton.of(18, 18, Buttons.NAVIGATE_UP, ButtonAction.of(
		  ...
		  .tooltip(Lists.newArrayList(
			 new TranslationTextComponent("simpleconfig.ui.navigate.edited.prev"),
			 new TranslationTextComponent(
				"simpleconfig.ui.shortcut",
				KeyBindings.PREV_EDITED.func_238171_j_()
				  .deepCopy().mergeStyle(TextFormatting.DARK_AQUA)))));
		navigateDownButton = MultiFunctionImageButton.of(18, 18, Buttons.NAVIGATE_DOWN, ButtonAction.of(
		  () -> focusNextEdited(true)
		).active(this::isEdited)
		  .tooltip(Lists.newArrayList(
			 new TranslationTextComponent("simpleconfig.ui.navigate.edited.next"),
			 new TranslationTextComponent(
				"simpleconfig.ui.shortcut",
				KeyBindings.NEXT_EDITED.func_238171_j_()
				  .deepCopy().mergeStyle(TextFormatting.DARK_AQUA)))));
	...
	@Override protected void init() {
		...
		addButton(navigateUpButton);
		navigateDownButton.setPosition(bx += 22, by);
		addButton(navigateDownButton);
	...
	@Override public void updateSelection() {
		...
		if (selecting != isSelecting) {
			if (selecting) {
				...
				remoteCommonButton.setExactWidth(20);
				if (bx + modeButtons.stream().mapToInt(Widget::getWidth).sum() > 0.35 * width)
					modeButtons.forEach(b -> b.setExactWidth(20));
				for (MultiFunctionIconButton b: modeButtons) {
					b.x = bx;
					bx += b.getWidth();
	...
	public void focusNextEdited(boolean forwards) {
		focusNextEntry(t -> {
			if (!(t instanceof AbstractConfigListEntry<?>) || !t.isEdited())
				return false;
			if (!(t instanceof CaptionedSubCategoryListEntry<?, ?>)) return true;
			AbstractConfigField<?> ce = ((CaptionedSubCategoryListEntry<?, ?>) t).getCaptionEntry();
	...
	protected static class SaveButton extends TintedButton {
		...
		public SaveButton(SimpleConfigScreen screen) {
			super(0, 0, 80, 20, NarratorChatListener.EMPTY, button -> {
				if (screen.isEditingConfigHotKey()) {
					...
					if (hasErrors) {
						screen.addDialog(ConfirmDialog.create(new TranslationTextComponent(
						  "simpleconfig.ui.save_with_errors.dialog.title"), d -> {
							d.setBody(splitTtc("simpleconfig.ui.save_with_errors.dialog.body"));
							d.setConfirmText(new TranslationTextComponent("simpleconfig.ui.save_with_errors"));
		...
		public void tick() {
			...
			setMessage(new TranslationTextComponent(
			  hasErrors? advanced.allow_save_with_errors
