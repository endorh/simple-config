...
/**
 * Kotlin class acting as proxy to all methods under [P], with default values,
 * Kotlin specific entry types and adapting some Kotlin types to make the
 * API friendlier.
 *
 * It's a class rather than an interface so the methods can be protected and
 * only accessible by inheritors of [SimpleKonfig], without polluting their
 * exposed API.
 */
open class SimpleKonfigBuilders internal constructor() {
     ...
     *
     * The keys can be any arbitrary non-collection entry (implementing [KeyEntryBuilder]).
     ...
     * By default, the key is a string entry and the value is an empty map
     */
    protected fun <V, C, G, Builder: ConfigEntryBuilder<V, C, G, Builder>> linkedMap(
      entry: Builder, value: Map<String, V> = emptyMap()
     ...
     * By default, the key is a string entry and the value is an empty map
     */
    protected fun <K, V, KC, C, KG, G, B : ConfigEntryBuilder<V, C, G, B>, KB> linkedMap(
      keyEntry: KB, entry: B, value: Map<K, V> = emptyMap()
    ) where KB : ConfigEntryBuilder<K, KC, KG, KB>, KB : KeyEntryBuilder<KG> =
