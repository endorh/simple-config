  ...
  val entries: Map<String, AbstractConfigEntry<*, *, *>>
    ...
    >(value, EntryType.unchecked(value::class.java)), DataClassEntryBuilder<B> {
        ...
        override fun buildEntry(parent: ConfigEntryHolder, name: String): DataClassEntry<B> {
            ...
            // this.entries.mapValues {
            //     DummyEntryHolder.build(parent, it.value)
            // }
    ...
    override fun doForPresentation(value: D): D {
        ...
        return super.doForPresentation(proxy.createFrom(value, Maps.transformEntries(
            Maps.filterValues(entries) { it!!.hasPresentation() }
        ) { name, v ->
...
class DataClassProxy<B: Any>(
  ...
  val bakedProperties: Map<String, B.() -> Any?> = emptyMap()
    ...
    private fun B.bakeProperties(): B = if (bakedProperties.keys.any { propertyMap[it] !is KMutableProperty<*> }) {
        createFrom(this, Maps.transformValues(bakedProperties) { it?.invoke(this) }, skipBake = true) { _, v -> v }
    } else apply {
