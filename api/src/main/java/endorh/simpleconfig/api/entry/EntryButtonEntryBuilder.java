...
public interface EntryButtonEntryBuilder<
  ...
  > extends GUIOnlyEntryBuilder<V, Gui, EntryButtonEntryBuilder<V, Gui, B>>, KeyEntryBuilder<Gui> {
	...
	@Contract(pure=true) EntryButtonEntryBuilder<V, Gui, B> label(ITextComponent label);
	
	@Contract(pure=true) EntryButtonEntryBuilder<V, Gui, B> label(Supplier<ITextComponent> label);
}
